...
* `SE_ENABLE_BROWSER_LEFTOVERS_CLEANUP`: default value `true`, set to `false` to completely disable the cleanup.
...
``` bash
$ docker run -e SE_ENABLE_BROWSER_LEFTOVERS_CLEANUP=false --shm-size="2g" selenium/node-chrome:4.18.1-20240224
```

With the previous command, the cleanup will be disabled altogether.
