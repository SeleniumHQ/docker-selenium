...
``` bash
$ docker run -d -p 4444:4444 -e SE_OPTS=-debug --name selenium-hub selenium/hub:3.0.1-aluminum
```
...
``` bash
$ docker run -d --name selenium-hub -p 4444:4444 selenium/hub:3.0.1-aluminum
$ CH=$(docker run --rm --name=ch \
    --link selenium-hub:hub -v /e2e/uploads:/e2e/uploads \
    selenium/node-chrome:3.0.1-aluminum)
```
...
$ FF=$(docker run --rm --name=fx \
    --link selenium-hub:hub -v /e2e/uploads:/e2e/uploads \
    selenium/node-firefox:3.0.1-aluminum)
```
...
``` bash
$ docker run -d -P -p <port4VNC>:5900 --link selenium-hub:hub selenium/node-chrome-debug:3.0.1-aluminum
$ docker run -d -P -p <port4VNC>:5900 --link selenium-hub:hub selenium/node-firefox-debug:3.0.1-aluminum
```
e.g.:
``` bash
$ docker run -d -P -p 5900:5900 --link selenium-hub:hub selenium/node-chrome-debug:3.0.1-aluminum
$ docker run -d -P -p 5901:5900 --link selenium-hub:hub selenium/node-firefox-debug:3.0.1-aluminum
```
...
And for standalone:
``` bash
$ docker run -d -p 4444:4444 -p <port4VNC>:5900 selenium/standalone-chrome-debug:3.0.1-aluminum
# OR
$ docker run -d -p 4444:4444 -p <port4VNC>:5900 selenium/standalone-firefox-debug:3.0.1-aluminum
```
or
``` bash
...
#FROM selenium/node-firefox-debug:3.0.1-aluminum
...
selenium/node-firefox           3.0.1-aluminum              69f762d0d79e        29 minutes ago      552.1 MB
selenium/node-chrome            3.0.1-aluminum              9dd73160660b        30 minutes ago      723.6 MB
selenium/node-base              3.0.1-aluminum              1b7a0b7024b1        32 minutes ago      426.1 MB
selenium/hub                    3.0.1-aluminum              2570bbb98229        33 minutes ago      394.4 MB
selenium/base                   3.0.1-aluminum              33478d455dab        33 minutes ago      362.6 MB
