...
Chrome
``` bash
$ docker run -d -p 4444:4444 -v /dev/shm:/dev/shm selenium/standalone-chrome:3.12.0-boron
#OR
$ docker run -d -p 4444:4444 --shm-size=2g selenium/standalone-chrome:3.12.0-boron
```
Firefox
``` bash
$ docker run -d -p 4444:4444 -v /dev/shm:/dev/shm selenium/standalone-firefox:3.12.0-boron
#OR
...
$ docker run -d -p 4444:4444 --net grid --name selenium-hub selenium/hub:3.12.0-boron
$ docker run -d --net grid -e HUB_HOST=selenium-hub -v /dev/shm:/dev/shm selenium/node-chrome:3.12.0-boron
$ docker run -d --net grid -e HUB_HOST=selenium-hub -v /dev/shm:/dev/shm selenium/node-firefox:3.12.0-boron
```
...
``` bash
$ docker run -d -p 4444:4444 --name selenium-hub selenium/hub:3.12.0-boron
$ docker run -d --link selenium-hub:hub -v /dev/shm:/dev/shm selenium/node-chrome:3.12.0-boron
$ docker run -d --link selenium-hub:hub -v /dev/shm:/dev/shm selenium/node-firefox:3.12.0-boron
```
...
``` bash
$ docker run -d -p 4444:4444 -e JAVA_OPTS=-Xmx512m --name selenium-hub selenium/hub:3.12.0-boron
```
...
``` bash
$ docker run -d -p 4444:4444 -e SE_OPTS="-debug" --name selenium-hub selenium/hub:3.12.0-boron
```
...
$ docker run -d -e HUB_HOST=<hub_ip|hub_name> -e HUB_PORT=4444 selenium/node-chrome:3.12.0-boron
```
...
``` bash
# Assuming a hub was already started
$ docker run -d -e HUB_HOST=<hub_ip|hub_name> -e REMOTE_HOST="http://node_ip|node_name:node_port" selenium/node-firefox:3.12.0-boron
```
...
$ CH=$(docker run --rm --name=ch \
    ...
    -v /dev/shm:/dev/shm \
    selenium/node-chrome:3.12.0-boron)
```
...
$ FF=$(docker run --rm --name=fx \
    --link selenium-hub:hub -v /e2e/uploads:/e2e/uploads \
    -v /dev/shm:/dev/shm \
    selenium/node-firefox:3.12.0-boron)
```
...
``` bash
$ docker run -d -P -p <port4VNC>:5900 --link selenium-hub:hub -v /dev/shm:/dev/shm selenium/node-chrome-debug:3.12.0-boron
$ docker run -d -P -p <port4VNC>:5900 --link selenium-hub:hub -v /dev/shm:/dev/shm selenium/node-firefox-debug:3.12.0-boron
```
e.g.:
``` bash
...
``` dockerfile
#FROM selenium/node-chrome-debug:3.12.0-boron
