...
BASE_RELEASE := $(or $(BASE_RELEASE),$(BASE_RELEASE),selenium-4.45.0)
BASE_VERSION := $(or $(BASE_VERSION),$(BASE_VERSION),4.45.0)
BINDING_VERSION := $(or $(BINDING_VERSION),$(BINDING_VERSION),4.45.0)
BASE_RELEASE_NIGHTLY := $(or $(BASE_RELEASE_NIGHTLY),$(BASE_RELEASE_NIGHTLY),nightly)
BASE_VERSION_NIGHTLY := $(or $(BASE_VERSION_NIGHTLY),$(BASE_VERSION_NIGHTLY),4.46.0-SNAPSHOT)
VERSION := $(or $(VERSION),$(VERSION),4.45.0)
...
test_k8s_autoscaling_job_count_strategy_default:
	MATRIX_TESTS=$(or $(MATRIX_TESTS), "AutoscalingTestsScaleUp") SCALING_STRATEGY=$(or $(SCALING_STRATEGY), "") TEST_MULTIPLE_PLATFORMS=true \
