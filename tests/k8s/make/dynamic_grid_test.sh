...
# With SE_VIDEO_SESSION_SUBFOLDER=true (set on the Dynamic Grid Deployments), the recorder writes
# each session's video directly into an <assets>/<sessionId>/ subfolder. Assert every video landed
# in such a subfolder, never flat in the assets root where same-named recordings would collide.
echo "Verify recordings are stored in per-session subfolders"
