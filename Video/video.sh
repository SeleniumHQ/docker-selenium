...
# Choose the recording mode by reachability of the session source:
#  - reachable  -> session-aware recording (discover the session; unified naming + subfolder toggle).
#    For the Docker external video container the assets path is already bound per session, so the
#    recording lands in <assets>/<sessionId>/ without needing the subfolder toggle.
