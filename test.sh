...
HUB=$(docker run -d selenium/hub:3.0.0-beta3)
...
NODE_CHROME=$(docker run -d --link $HUB_NAME:hub -e no_proxy="" selenium/node-chrome$DEBUG:3.0.0-beta3)
echo 'Starting Selenium Firefox node...'
NODE_FIREFOX=$(docker run -d --link $HUB_NAME:hub -e no_proxy="" selenium/node-firefox$DEBUG:3.0.0-beta3)
